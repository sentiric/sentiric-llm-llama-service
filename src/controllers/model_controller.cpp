...
void ModelController::handle_get_models(const httplib::Request &,
  ...
  json response_body = {
      {"object", "list"},
      {"data",
       {{{"id", current_settings.model_id.empty() ? "local-model"
                                                  : current_settings.model_id},
         {"object", "model"},
...
void ModelController::handle_switch_model(const httplib::Request &req,
                                          httplib::Response &res) {
  res.set_header("Access-Control-Allow-Origin", "*");
  ...
  try {
    ...
    if (profile.empty()) {
      res.status = 400;
      res.set_content(json({{"error", "Profile name required"}}).dump(),
                      "application/json");
      return;
    }
    ...
    if (success) {
      res.status = 200;
      res.set_content(
          json({{"status", "success"}, {"active_profile", profile}}).dump(),
          "application/json");
    } else {
      res.status = 500;
      res.set_content(
          json({{"status", "error"},
                {"message",
                 "Model reload failed. Profile not found or download error."}})
              .dump(),
          "application/json");
    }
...
void ModelController::handle_get_profiles(const httplib::Request &,
                                          httplib::Response &res) {
  res.set_header("Access-Control-Allow-Origin", "*");
  try {
    ...
    res.status = 500;
    res.set_content(json({{"error", "Could not read profiles.json"}}).dump(),
                    "application/json");
  }
}