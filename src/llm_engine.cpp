...
// --- Constructor, Destructor ve is_model_loaded (DEĞİŞİKLİK YOK) ---
LLMEngine::LLMEngine(Settings& settings) : settings_(settings) {
    ...
    try {
        ...
        model_loaded_ = true;
    } catch (const std::exception& e) { llama_model_free(model_); throw std::runtime_error(std::string("Context pool initialization failed: ") + e.what()); }
...
// --- generate_stream (Dinamik Parametreler Eklenmiş Stabil Versiyon) ---
void LLMEngine::generate_stream(
    ...
    for(size_t i = 0; i < prompt_tokens.size(); ++i) { common_batch_add(batch, prompt_tokens[i], i, {0}, false); }
    ...
    while (n_past < (int)llama_n_ctx(ctx) && n_decoded < max_tokens) {
        ...
        if (n_piece > 0) {
            ...
            for (const auto& stop_word : stop_sequences) {
                size_t pos = stop_buffer.find(stop_word);
                if (pos != std::string::npos) {
                    flush_str = stop_buffer.substr(0, pos);
                    stopping = true;
                    break;
                }
            }
        ...
        if (llama_decode(ctx, batch) != 0) { llama_batch_free(batch); llama_sampler_free(sampler_chain); throw std::runtime_error("Failed to decode next token"); }
        n_past++;
        n_decoded++;
    }
