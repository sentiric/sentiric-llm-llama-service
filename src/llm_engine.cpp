...
LLMEngine::LLMEngine(Settings& settings, prometheus::Gauge& active_contexts_gauge)
    ...
    spdlog::info("🚀 Initializing LLM Engine (Fixed Version)...");
...
void LLMEngine::process_batch(std::vector<std::shared_ptr<BatchedRequest>>& batch) {
    ...
    for (auto& req_ptr : batch) {
        try {
            ...
            std::vector<llama_token> prompt_tokens(formatted_prompt.length() + 16);
            ...
            // DÜZELTİLDİ: Pointer hatası giderildi.
            context_pool_->release(ctx, guard.get_id(), final_tokens);
