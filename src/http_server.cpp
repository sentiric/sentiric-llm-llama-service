...
HttpServer::HttpServer(std::shared_ptr<LLMEngine> engine,
                       const std::string &host, int port, int threads)
...
void HttpServer::run() {
  spdlog::info("📊 HTTP sunucusu (Health + Studio) {}:{} adresinde dinleniyor.",
               host_, port_);
  svr_.listen(host_.c_str(), port_);
}
