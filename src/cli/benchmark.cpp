...
BenchmarkResult Benchmark::run_performance_test(int iterations, const std::string& prompt) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        bool success = client_->generate_stream(prompt,
            [&](const std::string& /*token*/) { current_run_tokens++; });
        ...
        if (success) { result.successful_requests++; }
    ...
    if (duration_sec > 0) { result.tokens_per_second = total_tokens_generated / duration_sec; } else { result.tokens_per_second = 0; }
...
BenchmarkResult Benchmark::run_concurrent_test(int concurrent_connections, int requests_per_connection) {
    spdlog::info("⚡ Eşzamanlı test başlıyor ({} bağlantı, {} istek/bağlantı)...", concurrent_connections, requests_per_connection);
    BenchmarkResult result;
    ...
    for (int i = 0; i < concurrent_connections; i++) {
        futures.push_back(std::async(std::launch::async, [this, requests_per_connection, &success_count, &total_tokens, &total_latency_sum]() {
            ...
            for (int j = 0; j < requests_per_connection; j++) {
                ...
                if (success) { success_count++; total_tokens += tokens_in_req; total_latency_sum = total_latency_sum.load() + latency; }
            }
        }));
    }
    ...
    if (result.successful_requests > 0) { result.average_response_time_ms = total_latency_sum.load() / result.successful_requests; } else { result.average_response_time_ms = 0; }
    if (duration_sec > 0) { result.tokens_per_second = result.total_tokens_generated / duration_sec; } else { result.tokens_per_second = 0; }
...
void Benchmark::generate_report(const BenchmarkResult& result, const std::string& filename) {
    ...
    std::ofstream file;
    if (!filename.empty()) { file.open(filename); output = &file; }
    ...
    *output << "========================\n";
    ...
    if (file.is_open()) { file.close(); spdlog::info("Rapor dosyaya kaydedildi: {}", filename); }
}
