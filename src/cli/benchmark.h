...
class Benchmark {
 public:
  ...
  BenchmarkResult run_performance_test(
      int iterations = 10, const std::string& prompt = "Test prompt");

  BenchmarkResult run_concurrent_test(int concurrent_connections = 5,
                                      int requests_per_connection = 10);

  void run_interrupt_test(const std::string& initial_prompt,
                          const std::string& interrupt_prompt);

  void generate_report(const BenchmarkResult& result,
