...
struct fmt::formatter<grpc::StatusCode> {
  ...
  auto format(const grpc::StatusCode& code,
...
std::string read_file_client(const std::string& filepath) {
  ...
  if (!file.is_open()) {
    throw std::runtime_error("File could not be opened for client: " +
                             filepath);
  }
...
GRPCClient::GRPCClient(const std::string& endpoint) : endpoint_(endpoint) {}
...
void GRPCClient::ensure_channel_is_ready() {
  ...
  try {
    const char *ca = std::getenv("GRPC_TLS_CA_PATH"),
               *cert = std::getenv("LLM_LLAMA_SERVICE_CERT_PATH"),
               *key = std::getenv("LLM_LLAMA_SERVICE_KEY_PATH");
    if (!ca || !cert || !key || access(ca, R_OK) != 0 ||
        access(cert, R_OK) != 0 || access(key, R_OK) != 0) {
      creds = grpc::InsecureChannelCredentials();
    } else {
      grpc::SslCredentialsOptions opts = {
          read_file_client(ca), read_file_client(key), read_file_client(cert)};
...
bool GRPCClient::is_connected() {
  ...
  return channel_->WaitForConnected(gpr_time_add(
...
bool GRPCClient::generate_stream_with_cancellation(
  ...
  cancellable_context_->set_deadline(std::chrono::system_clock::now() +
...
bool GRPCClient::generate_stream(
  ...
  context.set_deadline(std::chrono::system_clock::now() +
                       std::chrono::seconds(timeout_seconds_));
  context.AddMetadata("x-trace-id",
                      "cli-test-" + std::to_string(std::time(nullptr)));
  ...
  while (reader->Read(&response)) {
    ...
    if (response.has_finish_details()) {
      ...
      spdlog::info(
          ...
          d.finish_reason(), d.prompt_tokens(), d.completion_tokens());
    }
  }
  auto status = reader->Finish();
  if (!status.ok() && status.error_code() != grpc::StatusCode::CANCELLED) {
...
void GRPCClient::set_timeout(int seconds) { timeout_seconds_ = seconds; }
