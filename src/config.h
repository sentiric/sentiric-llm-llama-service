...
struct Settings {
  ...
  std::string model_url_template =
      "https://huggingface.co/{model_id}/resolve/main/{filename}";

  // --- TEMPLATE CONFIGURATION (TURKISH DEFAULT / ADAPTIVE) ---
  // [ADAPTIVE] Türkçe varsayılan, İngilizce algılandığında geçiş yapan akıllı
  // prompt
  std::string template_system_prompt =
      "Sen 'Sentirik'sin. Profesyonel, yardımsever ve zeki bir asistansın.\n"
      "\n"
      "### İLETİŞİM PROTOKOLLERİ:\n"
      "1. DİL: Varsayılan dilin Türkçe'dir. Ancak kullanıcı İngilizce veya "
      "başka bir dilde yazarsa, algıladığın dilde yanıt ver.\n"
      "2. ÜSLUP: Doğal, akıcı ve insan benzeri konuş. Robotik kalıplardan "
      "(Örn: 'Bir yapay zeka olarak...') kaçın.\n"
      "3. RAG: Eğer [BİLGİ] bloğu verilmişse, cevabı SADECE oradaki veriye "
      "dayandır.\n"
      "4. GÜVENLİK: Tıbbi/Hayati acil durumlarda yorum yapma, doğrudan "
      "112/Doktora yönlendir.";
  ...
  std::string reasoning_prompt_low =
      "\n[TALİMAT]: Cevap vermeden önce kısaca düşün. Düşüncelerini <think> "
      "etiketleri içine al.";
  std::string reasoning_prompt_high =
      "\n[TALİMAT]: Bu karmaşık bir görev. Adım adım derinlemesine düşün. "
      "Analizini <think> etiketleri içine al.";

  // --- ENGINE & HARDWARE PERFORMANCE (ECO MODE DEFAULTS) ---
  int n_gpu_layers = 100;  // Default: All Layers on GPU (Speed)
  uint32_t context_size =
      2048;  // Default: 2048 (VRAM Safety for Multi-Service)

  // Threading Strategy (Limited to 4 to save CPU for STT/TTS)
  // Hardware concurrency genellikle sanal çekirdekleri de sayar, 4 güvenli bir
  // limittir.
  uint32_t n_threads =
...
// ==================================================================================
inline bool apply_profile(Settings& s,
                          const std::string& profile_name_override) {
  ...
  try {
    ...
    if (j.contains("profiles") && j["profiles"].contains(active)) {
      ...
      if (p.contains("physical_batch_size"))
        s.physical_batch_size = p["physical_batch_size"];
      ...
      if (p.contains("enable_batching"))
        s.enable_dynamic_batching = p["enable_batching"];
      ...
      if (p.contains("repeat_penalty"))
        s.default_repeat_penalty = p["repeat_penalty"];

      // --- Templates ---
      if (p.contains("templates")) {
        const auto& templates = p["templates"];
        s.template_system_prompt =
            templates.value("system_prompt", s.template_system_prompt);
        s.template_rag_prompt =
            templates.value("rag_prompt", s.template_rag_prompt);
      }

      return true;
    } else {
      spdlog::warn(
          "⚠️ Requested profile '{}' not found in profiles.json. Falling back "
          "to defaults.",
          active);
      return false;
    }
...
inline Settings load_settings() {
  ...
  auto get_env_var = [](const char* name,
  ...
  auto override_bool = [&](const char* name, bool& current_val) {
    ...
    if (val_str) {
      ...
      spdlog::info("🔧 [Env Override] {} = {}", name,
                   current_val ? "true" : "false");
    }
  };
  ...
  override_float("LLM_LLAMA_SERVICE_DEFAULT_TEMPERATURE",
  ...
  override_float("LLM_LLAMA_SERVICE_DEFAULT_REPEAT_PENALTY",
                 s.default_repeat_penalty);
  override_string("LLM_LLAMA_SERVICE_DEFAULT_SYSTEM_PROMPT",
                  s.template_system_prompt);
  override_string("LLM_LLAMA_SERVICE_DEFAULT_RAG_PROMPT",
                  s.template_rag_prompt);
  override_string("LLM_LLAMA_SERVICE_DEFAULT_RAG_PROMPT_LOW",
                  s.reasoning_prompt_low);
  override_string("LLM_LLAMA_SERVICE_DEFAULT_RAG_PROMPT_HIGH",
                  s.reasoning_prompt_high);

  // Gateway
  s.worker_id =
      get_env_var("LLM_WORKKER_ID", "worker-" + std::to_string(std::rand()));
  ...
  if (s.context_size < 512) {
    spdlog::warn("⚠️ Context size {} is too low. Forcing minimum 512.",
                 s.context_size);
    s.context_size = 512;
  }
