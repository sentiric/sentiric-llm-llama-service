...
namespace suts {
    ...
    public:
        void format(const spdlog::details::log_msg &msg, spdlog::memory_buf_t &dest) override {
            ...
            j["resource"] = {
                {"service.name", "llm-llama-service"},
...
// [ARCH-COMPLIANCE] C++ Makroları
#define SUTS_INFO(event, tid, sid, ten, ...) spdlog::info(suts::build(event, tid, sid, ten, __VA_ARGS__))
#define SUTS_ERROR(event, tid, sid, ten, ...) spdlog::error(suts::build(event, tid, sid, ten, __VA_ARGS__))
#define SUTS_WARN(event, tid, sid, ten, ...) spdlog::warn(suts::build(event, tid, sid, ten, __VA_ARGS__))
#define SUTS_DEBUG(event, tid, sid, ten, ...) spdlog::debug(suts::build(event, tid, sid, ten, __VA_ARGS__))